...
- Plugin version is canonical in `.claude-plugin/plugin.json`; `VERSION` mirrors it. Bump both atomically; divergence between them is the bug.
...
| `.agents/rules/plugin-packaging.md` | `.claude-plugin/**`, `.codex-plugin/**`, `.agents/plugins/**`, `evals/**`, `VERSION`, `CHANGELOG.md`, `pyproject.toml` | Version sync (3 files), dual manifest, dual marketplace catalog, marketplace-only install, changelog format, eval suite |
